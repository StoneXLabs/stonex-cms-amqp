...
AMQP_DEFINES
	...
	class CMS_API TemporaryQueue : public cms::TemporaryQueue
	{
        friend class MessageConsumerImpl;//remove
        friend class MessageProducer;//remove
    public:
        explicit TemporaryQueue(const std::string& queueName = "");
        TemporaryQueue(const TemporaryQueue& other);
        TemporaryQueue(TemporaryQueue&& other) = delete;

        TemporaryQueue& operator=(const TemporaryQueue& other);
