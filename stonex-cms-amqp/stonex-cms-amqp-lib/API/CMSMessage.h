...
AMQP_DEFINES
	...
	class CMS_API CMSMessage : public ::cms::Message
	{
		friend class MessageProducerImpl;
	public:
		CMSMessage();
		CMSMessage(const CMSMessage& other);
		CMSMessage(CMSMessage&& other) = delete;

		explicit CMSMessage(proton::message* mes);
		CMSMessage(proton::message* mes, const proton::sender* sender);
		CMSMessage(proton::message* mes, const proton::receiver* receiver);

		~CMSMessage() override = default;
